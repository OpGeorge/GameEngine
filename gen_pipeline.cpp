...
namespace gen {
	...
	GenPipeline::GenPipeline(GenDevice& device,
	...
	void GenPipeline::createGraphicsPipeline(const std::string& vertFilePath, const std::string& fragFilePath, const PipelineConfigInfo& configInfo) {
		...
		auto& bindingDescriptions = configInfo.bindingDescripstions;
		auto& attributeDescriptions = configInfo.attributeDescripstions;
		...
		if (vkCreateGraphicsPipelines(genDevice.device(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS) {
			throw std::runtime_error("failed to create grahphics pipeline!\n\n\n");
		}
	...
	void GenPipeline::createShaderModule(const std::vector<char>& code, VkShaderModule* shaderModule) {
		...
		if (vkCreateShaderModule(genDevice.device(), &createInfo, nullptr, shaderModule) != VK_SUCCESS) {
			throw std::runtime_error("failed to create shader Modulen\n\n");
		}
	...
	void GenPipeline::defaultPipelineConfigInfo(PipelineConfigInfo& configInfo) {
		...
		configInfo.bindingDescripstions = GenModel::Vertex::getBindingDescriptions();
		configInfo.attributeDescripstions = GenModel::Vertex::getAttributeDescriptions();
